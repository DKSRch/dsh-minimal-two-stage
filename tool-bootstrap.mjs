 ...
 *
 * LOCAL minimal-two-stage OPTION: `workspaceLine: false` skips the promoted
 * cwd append (`withWorkspaceLine`) so phase 2 opens the tool catalog while
 * the system prompt stays byte-identical to the phase-1 anchor.
 *
...
export function apply(ctx, config) {
  ...
  // LOCAL minimal-two-stage option: skip the promoted cwd append so phase 2
  // adds tools without adding any prompt text. Default true keeps upstream
  // behavior; the minimal-two-stage composition sets it to false.
  const workspaceLine = config.workspaceLine !== false
  const policy = {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    if (state.promoted) return workspaceLine ? withWorkspaceLine(assembled, agent) : assembled
